...
#    2026-07 改「順勢個性」配置：順勢重倉、籌碼重（散戶行為是反指標）、
#    輕總經與估值（不猜景氣、不抄底）。
#    ⚠ 誠實註記：截至改版當下，回測「支柱 IC」顯示總經是唯一明顯有預測力
#    的支柱（3月 IC≈+0.36）、趨勢趨近 0——這份權重是風格選擇而非證據選擇；
#    表現好壞由回測頁的體檢卡/IC 卡持續記錄，用數據回頭修。
#    （舊配置備查：fear22 / valuation20 / macro14 / trend24 / chips20）
PILLAR_WEIGHTS = {
    "fear": 16,        # 恐慌與情緒（人棄我取，但不主導）
    "valuation": 10,   # 估值（不為便宜而買）
    "macro": 12,       # 總經（看戲成分居多，留一點當煞車）
    "trend": 34,       # 趨勢動能 + 技術面（主菜：趨勢在就參與）
    "chips": 28,       # 籌碼：法人 vs 散戶（別當追高的那隻韭菜）
}
...
INDICATOR_WEIGHTS = {
    ...
    # 趨勢・技術面（由趨勢品質/技術指標主導；舊 trend_*/rel 已隨 bias 機制移除）
...
# band 名改「市場溫度」語彙（網站定位＝觀察大盤，不再給扣款指令）；
# 倍數仍保留給 strategy_backtest 的主動策略模擬用，前台不顯示。
ACTION_BANDS = [
    (70, "遍地黃金區", "數據難得排這麼整齊——恐慌給的折扣，歷史上都是事後才看得懂的禮物。", 2.0),
    (58, "機會偏多區", "數據站在買方這邊。該貪的時候別慫，但貪也要分批，不是歐印。", 1.5),
    (45, "中性區",     "不上不下，最無聊但最不會出事的位置。沒事就把手機關掉。", 1.0),
    (35, "偏熱謹慎區", "市場開始擠了。手癢想追的，先想想上次追高的下場。", 0.5),
    (0,  "過熱危險區", "全場都樂觀的時候，剩下能買的人不多了。逞英雄沒獎品。", 0.25),
]

# 4b) 綜合分數「對比」旋鈕：分數常被一堆窄區間指標拉回 50，導致「積極加碼/保守」
#     幾乎不會觸發、定額倍數老是卡在 1.0~1.5。此值 >1 會以 50 為中心把分數拉開：
#       composite = 50 + (composite-50) * COMPOSITE_CONTRAST  （夾在 0-100）
#     1.0＝完全不變（預設、安全）；想更敢喊就調 1.2~1.4，但務必用 forecast.py 回測，
#     確認沒有增加假訊號/反指標再上線。
...
STOCK_KLINE_DAYS = 60              # 個股卡 K 線顯示的交易日數（含 5/20 均線）
...
THEMES = {
    ...
    "功率元件": ["2481", "5425", "8255", "3675", "6435"],
    "電源/BBU": ["2308", "6282", "3211", "3323", "6781"],
    "玻纖/CCL": ["2383", "6274", "6213", "1815"],
    "光通訊": ["4977", "6451", "3081"],
    "無人機": ["8033", "5371"],
    "高速傳輸": ["3665", "3533"],
}
# ※ 題材會輪動：這張表是「命名」用，熱不熱由 stock.theme_heat() 每天用
#   實際動能＋法人買超算出來，冷掉的題材自然不會被選股引用；但全新題材
#   要記得來這裡加一行，選股才叫得出它的名字。
